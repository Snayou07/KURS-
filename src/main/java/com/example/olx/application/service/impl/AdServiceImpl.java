...
public class AdServiceImpl implements AdServicePort {
    ...
    // УЯВІМО, ЩО У ВАС Є ТАКИЙ КОД (ДО ЗМІН):
    public void saveAd_OLD(Ad ad, boolean isPremium, boolean isUrgent) {
        String description = ad.getDescription();
        String metadata = "premium:" + isPremium + ";urgent:" + isUrgent + ";";
        String fullDescription = description + "\n\n---DECORATORS---\n" + metadata;
        ad.setDescription(fullDescription);

        // ... логіка збереження в базу даних / файл ...
    }


    // ЯК ЦЕЙ КОД МАЄ ВИГЛЯДАТИ ЗАРАЗ (ПІСЛЯ ЗМІН):
    public void saveAd_NEW(Ad ad, boolean isPremium, boolean isUrgent, String deliveryInfo) {
        // 1. Опис залишається чистим
        // ad.setDescription("Текст опису від користувача"); - це вже має бути в об'єкті ad

        // 2. Створюємо і заповнюємо мапу декораторів
        Map<String, String> decorators = new HashMap<>();
        decorators.put("premium", String.valueOf(isPremium)); // "true" або "false"
        decorators.put("urgent", String.valueOf(isUrgent));   // "true" або "false"
        if (deliveryInfo != null && !deliveryInfo.isEmpty()) {
            decorators.put("deliveryInfo", deliveryInfo);
        }
        // ... додаємо інші декоратори за такою ж логікою ...

        // 3. Встановлюємо мапу в об'єкт Ad
        ad.setDecorators(decorators);

        // 4. ... логіка збереження в базу даних / файл ...
        // Тепер ви зберігаєте поле `description` і поле `decorators` (наприклад, у JSON колонку)
    }
